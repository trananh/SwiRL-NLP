...
Sentence DocumentSerializer::loadSentence(istream &stream) {
    ...
    while (offset < tokenCount) {
    	...
    	if (tokens.at(3).compare(&NIL) != 0) nilTags = false;
    	lemmaBuffer.push_back(tokens.at(4));
    	if (tokens.at(4).compare(&NIL) != 0) nilLemmas = false;
    	entityBuffer.push_back(tokens.at(5));
    	if (tokens.at(5).compare(&NIL) != 0) nilEntities = false;
    	normBuffer.push_back(tokens.at(6));
    	if (tokens.at(6).compare(&NIL) != 0) nilNorms = false;
    	chunkBuffer.push_back(tokens.at(7));
    	if (tokens.at(7).compare(&NIL) != 0) nilChunks = false;
    ...
    assert((int)lemmaBuffer.size() == 0 || (int)lemmaBuffer.size() == tokenCount);
    assert((int)entityBuffer.size() == 0 || (int)entityBuffer.size() == tokenCount);
    assert((int)normBuffer.size() == 0 || (int)normBuffer.size() == tokenCount);
    assert((int)chunkBuffer.size() == 0 || (int)chunkBuffer.size() == tokenCount);
 ...
 * 	@param out The output stream.
 */
void DocumentSerializer::saveToken(Sentence sentence, int offset, ostream &out) {
	out << sentence.words.at(offset) + SEP <<
			sentence.startOffsets.at(offset) << SEP <<
			sentence.endOffsets.at(offset);

	out << SEP;
	if (sentence.tags.size() > 0) out << sentence.tags.at(offset);
	else out << NIL;

	out << SEP;
	if (sentence.lemmas.size() > 0) out << sentence.lemmas.at(offset);
	else out << NIL;

	out << SEP;
	if (sentence.entities.size() > 0) out << sentence.entities.at(offset);
	else out << NIL;

	out << SEP;
	if (sentence.norms.size() > 0) out << sentence.norms.at(offset);
	else out << NIL;

	out << SEP;
	if (sentence.chunks.size() > 0) out << sentence.chunks.at(offset);
	else out << NIL;
